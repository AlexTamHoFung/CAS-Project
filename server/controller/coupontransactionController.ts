...
export class CoupontransactionsController {
	...
	getCoupontransaction = async (req: Request, res: Response) => {
		try {
			...
			const coupontransactionResult =
				await this.coupontransactionsService.getCoupontransaction(customer_id);

			if (coupontransactionResult.length > 0) {
				res.json({
					message: "found coupon transaction",
					data: coupontransactionResult
				});
				return;
			} else {
	...
	createCoupontransaction = async (req: Request, res: Response) => {
		const { total, quantity, transaction_date, listing_id, customer_id } =
			req.body;
		const coupontransaction =
			await this.coupontransactionsService.createCoupontransaction(
				total,
				quantity,
				transaction_date,
				listing_id,
